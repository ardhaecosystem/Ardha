...
class TestChatRepository:
    ...
    async def test_create_chat_success(self, test_db):
        ...
        chat = await repo.create(
            user_id=user_id, mode=mode, project_id=project_id, title="Test Chat"
        )
    ...
    async def test_get_by_project_success(self, test_db):
        ...
        chat1 = await repo.create(
            user_id=user_id, mode=ChatMode.RESEARCH.value, project_id=project_id
        )
