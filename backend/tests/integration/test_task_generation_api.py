...
from ardha.schemas.workflows.task_generation import (
    TaskGenerationState, TaskGenerationWorkflowConfig
)
...
class TestTaskGenerationAPI:
    ...
    def test_execute_task_generation_success(self, client, mock_user, mock_workflow):
        ...
        # Mock dependencies
        with patch('ardha.api.v1.routes.task_generation.get_current_active_user', return_value=mock_user):
            with patch('ardha.api.v1.routes.task_generation.get_task_generation_workflow', return_value=mock_workflow):
    ...
    def test_get_task_generation_status_success(self, client, mock_user, mock_workflow):
        ...
        assert abs(data["quality_score"] - 0.16) < 1e-10  # Weighted average with only prd_analysis_quality
    ...
    def test_get_task_generation_status_access_denied(self, client, mock_workflow):
        ...
        with patch('ardha.api.v1.routes.task_generation.get_current_active_user', return_value=current_user):
            with patch('ardha.api.v1.routes.task_generation.get_task_generation_workflow', return_value=mock_workflow):
                response = client.get(f"/api/v1/task-generation/{execution_id}/status")
    ...
    def test_cancel_task_generation_success(self, client, mock_user, mock_workflow):
        ...
        try:
            with patch('ardha.api.v1.routes.task_generation.get_task_generation_workflow', return_value=mock_workflow):
                response = client.delete(f"/api/v1/task-generation/{execution_id}?reason=Test cancellation")
        finally:
            # Clear override
            app.dependency_overrides.clear()
    ...
    def test_list_task_generation_executions_with_filters(self, client, mock_user):
        ...
        try:
            response = client.get("/api/v1/task-generation/executions?limit=10&offset=5&status=completed")
        finally:
            # Clear override
            app.dependency_overrides.clear()
            ...
            with patch.object(workflow.nodes["breakdown_tasks"], '_call_ai') as mock_breakdown:
                with patch.object(workflow.nodes["define_dependencies"], '_call_ai') as mock_deps:
                    with patch.object(workflow.nodes["estimate_effort"], '_call_ai') as mock_effort:
                        with patch.object(workflow.nodes["generate_openspec"], '_call_ai') as mock_openspec:
                            ...
                            # Mock other methods
                            with patch.object(workflow.nodes["analyze_prd"], '_get_relevant_context', return_value=[]):
                                with patch.object(workflow.nodes["analyze_prd"], '_store_memory'):
                                    with patch.object(workflow.nodes["breakdown_tasks"], '_get_relevant_context', return_value=[]):
                                        with patch.object(workflow.nodes["breakdown_tasks"], '_store_memory'):
                                            with patch.object(workflow.nodes["define_dependencies"], '_get_relevant_context', return_value=[]):
                                                with patch.object(workflow.nodes["define_dependencies"], '_store_memory'):
                                                    with patch.object(workflow.nodes["estimate_effort"], '_get_relevant_context', return_value=[]):
                                                        with patch.object(workflow.nodes["estimate_effort"], '_store_memory'):
                                                            with patch.object(workflow.nodes["generate_openspec"], '_get_relevant_context', return_value=[]):
                                                                with patch.object(workflow.nodes["generate_openspec"], '_store_memory'):
                                                                    with patch.object(workflow, '_get_workflow_context'):
