...
from ardha.schemas.workflows.task_generation import TaskGenerationWorkflowConfig
from ardha.schemas.workflows.task_generation import TaskGenerationState
...
class TestTaskGenerationWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_openspec_file_generation(
        self,
        sample_prd_content,
        sample_project_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_task_database_integration(
        self,
        sample_project_context,
        ...
        from unittest.mock import patch
        with patch('ardha.services.task_generation_service.async_session_factory') as mock_session_factory:
    ...
    @pytest.mark.asyncio
    async def test_openspec_file_validation(
        self
    ):
    ...
    async def test_workflow_progress_tracking(
        ...
        expected_steps = ["analyze_prd", "breakdown_tasks", "define_dependencies", "estimate_effort", "generate_openspec"]
    ...
    @pytest.mark.asyncio
    async def test_openspec_proposal_listing(
        self
    ):
        ...
        test_proposal = next(
            (p for p in proposals if p.get("proposal_id") == "list-test-proposal"),
            None
        )
    ...
    @pytest.mark.asyncio
    async def test_openspec_archival(
        self
    ):
        ...
        found_in_active = any(p.get("proposal_id", "").startswith(unique_id) for p in active_proposals)
        
        assert not found_in_active, f"Archived proposal still found in active proposals: {[p.get('proposal_id') for p in active_proposals]}"
        ...
        archived_proposal = next(
            (p for p in all_proposals if p.get("proposal_id") == unique_id),
            None
        )
