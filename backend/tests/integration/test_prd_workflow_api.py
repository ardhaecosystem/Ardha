...
def mock_user_session():
    ...
    return {"user_id": uuid4(), "email": "test@example.com", "is_active": True}
...
class TestPRDWorkflowAPI:
    ...
    @pytest.mark.asyncio
    async def test_start_prd_workflow_success(
        self, client, mock_user_session, sample_research_summary, mock_prd_workflow
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_prd_workflow_result_not_completed(
        self, client, mock_user_session, mock_prd_workflow
    ):
    ...
    async def test_prd_workflow_error_handling(self, client, mock_user_session, mock_prd_workflow):
        ...
        with patch("ardha.workflows.prd_workflow.get_prd_workflow", return_value=mock_prd_workflow):
            ...
            response = client.post(
                "/api/v1/workflows/prd/start",
                json={"research_summary": {"idea": "Test idea"}, "project_id": str(uuid4())},
                headers={"Authorization": f"Bearer {mock_user_session['user_id']}"},
            )
    ...
    @pytest.mark.asyncio
    async def test_prd_workflow_with_custom_config(
        self, client, mock_user_session, sample_research_summary, mock_prd_workflow
    ):
...
class TestPRDWorkflowStreaming:
    ...
    @pytest.mark.asyncio
    async def test_prd_workflow_progress_streaming(
        self, client, mock_user_session, mock_prd_workflow
    ):
