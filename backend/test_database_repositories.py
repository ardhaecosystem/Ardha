...
async def test_method_counts():
    ...
    try:
        ...
        prop_methods = [
            ...
            "reorder",
            "get_by_type",
            "get_formula_properties",
            "get_rollup_properties",
...
async def test_model_references():
    ...
    try:
        ...
        ), "DatabaseProperty missing database relationship"
        ...
        assert hasattr(
            DatabaseEntryValue, "property"
        ), "DatabaseEntryValue missing property relationship"
...
async def test_async_patterns():
    ...
    try:
        ...
        for repo_class in [DatabaseRepository, DatabasePropertyRepository, DatabaseEntryRepository]:
            for name, method in getmembers(repo_class, predicate=isfunction):
                if not name.startswith("_") and name != "__init__":
                    assert iscoroutinefunction(method), f"{repo_class.__name__}.{name} is not async"
