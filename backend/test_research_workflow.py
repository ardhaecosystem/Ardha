...
from ardha.schemas.workflows.research import ResearchState, ResearchWorkflowConfig
...
class TestProgressCallback:
    ...
    async def __call__(self, progress_update):
        ...
        logger.info(
            f"Progress: {progress_update.current_step} - {progress_update.progress_percentage}%"
        )
...
async def test_research_workflow_basic():
    ...
    try:
        ...
        result = await workflow.execute(
            idea=test_idea, user_id=user_id, progress_callback=progress_callback
        )
...
async def test_research_workflow_with_config():
    ...
    try:
        ...
        result = await workflow.execute(
            idea=test_idea, user_id=user_id, parameters={"test_mode": True}
        )
...
async def test_research_workflow_error_handling():
    ...
    try:
        ...
        config = ResearchWorkflowConfig(
            max_retries_per_step=1, timeout_per_step_seconds=30  # Short timeout for testing
        )
        workflow = ResearchWorkflow(config)
        ...
        result = await workflow.execute(idea=test_idea, user_id=user_id)
...
async def test_workflow_integration():
    ...
    try:
        ...
        for i, idea in enumerate(test_ideas[:1]):  # Test just one idea for now
            ...
            result = await workflow.execute(idea=idea, user_id=user_id, context={"test_run": True})
