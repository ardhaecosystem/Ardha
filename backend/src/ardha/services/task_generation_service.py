...
class TaskGenerationService:
        ...
        try:
            async with async_session_factory() as session:
                ...
                for task_data in task_breakdown:
                    ...
                    self.logger.info(f"Saved task: {task_identifier} - {task_data.get('title', 'Untitled')}")
        ...
        try:
            ...
            activity = TaskActivity(
                ...
                metadata={
                    ...
                    "change_directory": openspec_data.get("metadata", {}).get("change_directory_path"),
    ...
    def _create_task_id_mapping(self, task_breakdown: List[Dict[str, Any]], task_ids: List[UUID]) -> Dict[str, UUID]:
        """
        ...
        non_epic_tasks = [
            task for task in task_breakdown 
            if not task.get("is_epic", False)
        ]
    ...
    async def get_project_tasks_for_generation(
        self,
        project_id: UUID
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            async with async_session_factory() as session:
                ...
                self.logger.info(f"Retrieved {len(existing_tasks)} existing tasks for project {project_id}")
