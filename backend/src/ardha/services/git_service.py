...
class GitService:
    ...
    def initialize(self, initial_branch: str = "main") -> Dict:
        ...
        if self.is_initialized():
            raise GitRepositoryExistsError(
                f"Git repository already exists at {self.repo_path}"
            )
    ...
    def get_commit_history(
        ...
        try:
            ...
            # Get commits
            if file_path:
                commit_iter = self.repo.iter_commits(revision, max_count=max_count + skip, paths=[file_path])
            else:
