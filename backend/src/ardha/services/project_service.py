...
from ardha.schemas.requests.project import ProjectCreateRequest, ProjectUpdateRequest
...
class ProjectService:
    ...
    async def update_project(
        ...
        if not await self.check_permission(project_id, user_id, "admin"):
            logger.warning(f"User {user_id} lacks permission to update project {project_id}")
            raise InsufficientPermissionsError("Only project owner or admin can update project")
    ...
    async def archive_project(self, project_id: UUID, user_id: UUID) -> bool:
        ...
        # Check permissions (owner or admin)
        if not await self.check_permission(project_id, user_id, "admin"):
            logger.warning(f"User {user_id} lacks permission to archive project {project_id}")
            raise InsufficientPermissionsError("Only project owner or admin can archive project")
    ...
    async def delete_project(self, project_id: UUID, user_id: UUID) -> bool:
        ...
        if not await self.check_permission(project_id, user_id, "owner"):
            logger.warning(f"User {user_id} lacks permission to delete project {project_id}")
            raise InsufficientPermissionsError("Only project owner can delete project")
    ...
    async def add_member(
        ...
        if not await self.check_permission(project_id, requester_id, "admin"):
            ...
            raise InsufficientPermissionsError("Only project owner or admin can add members")
    ...
    async def remove_member(
        ...
        if not await self.check_permission(project_id, requester_id, "admin"):
            ...
            raise InsufficientPermissionsError("Only project owner or admin can remove members")
