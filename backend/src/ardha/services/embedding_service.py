...
class LocalEmbeddingService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Advanced features: Pool={self.settings.enable_embedding_pool}, Smart batching={self.settings.enable_smart_batching}"
        )
    ...
    async def get_embedding_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pool_hit_rate": (
                self._pool_hits / total_pool_requests if total_pool_requests > 0 else 0.0
            ),
            "pool_size": len(self._embedding_pool),
            "pool_max_size": self._pool_max_size,
            "average_time": (
                self._total_time / self._total_embeddings if self._total_embeddings > 0 else 0.0
            ),
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": (
                    "healthy"
                    if (model_status == "loaded" and generation_status == "working")
                    else "degraded"
                ),
