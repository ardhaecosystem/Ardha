...
class TaskService:
    ...
    async def get_task(self, task_id: UUID, user_id: UUID) -> Any:  # Returns Task
        ...
        if not await self.project_service.check_permission(
            ...
            raise InsufficientTaskPermissionsError("Must be a project member to view tasks")
    ...
    async def delete_task(self, task_id: UUID, user_id: UUID) -> bool:
        ...
        if not await self.project_service.check_permission(
            ...
            raise InsufficientTaskPermissionsError("Must be project admin or owner to delete tasks")
    ...
    async def assign_task(
        ...
        if not await self.project_service.check_permission(
            ...
            raise InsufficientTaskPermissionsError("Assignee must be a project member")
    ...
    async def add_dependency(
        ...
        if await self.check_circular_dependency(task_id, depends_on_task_id):
            raise CircularDependencyError(f"Adding dependency would create circular dependency")
    ...
    async def remove_dependency(
        ...
        if success:
            ...
            await self.repository.log_activity(
                ...
                old_value=(
                    depends_on_task.identifier if depends_on_task else str(depends_on_task_id)
                ),
            )
    ...
    async def add_tag_to_task(
        ...
        if not await self.project_service.check_permission(
            ...
            raise InsufficientTaskPermissionsError("Must be at least a project member to add tags")
