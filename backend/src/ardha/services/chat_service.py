...
from ardha.core.openrouter import OpenRouterError, CircuitBreakerOpenError
...
from ardha.services.project_service import ProjectService, InsufficientPermissionsError, ProjectNotFoundError

logger = logging.getLogger(__name__)

# System message templates for different AI modes
SYSTEM_MESSAGES = {
    ChatMode.RESEARCH: """You are a research assistant. Your role is to help users conduct thorough research, analyze information, and provide well-structured insights. You should:
...
- Structure information clearly with headings and bullet points""",
    ...
    ChatMode.ARCHITECT: """You are a software architect. Your role is to help design robust, scalable systems and make architectural decisions. You should:
...
- Recommend best practices and industry standards""",
    ...
    ChatMode.IMPLEMENT: """You are an expert developer. Your role is to help write high-quality, production-ready code. You should:
...
- Suggest improvements and optimizations""",
    ...
    ChatMode.DEBUG: """You are a debugging expert. Your role is to help identify, analyze, and resolve issues in code and systems. You should:
...
class ChatService:
    ...
    async def create_chat(
        ...
        try:
            ...
            raise InvalidChatModeError(f"Invalid mode: {mode}. Must be one of: {[m.value for m in ChatMode]}")
    ...
    async def send_message(
        ...
        if chat.user_id != user_id:
            raise InsufficientChatPermissionsError(
                f"User {user_id} does not own chat {chat_id}"
            )
        ...
        try:
            ...
            try:
                ...
                from ardha.schemas.ai.requests import StreamingRequest, ChatMessage as AIChatMessage
                from ardha.schemas.ai.requests import MessageRole as AIMessageRole
                
                streaming_request = StreamingRequest(
                    model=model,
                    messages=[AIChatMessage(role=AIMessageRole(msg["role"]), content=msg["content"]) for msg in messages],
                    temperature=0.7,
                    max_tokens=4000,
                )
    ...
    async def _check_chat_budget(self, chat: Chat) -> None:
        ...
        if daily_total >= daily_budget:
            raise ChatBudgetExceededError(
                f"Daily AI budget of ${daily_budget} exceeded. "
                f"Current usage: ${daily_total}"
            )
