...
task_task_tags = Table(
    ...
    Column("tag_id", PG_UUID(as_uuid=True), ForeignKey("task_tags.id", ondelete="CASCADE"), primary_key=True),
)
...
class Task(Base, BaseModel):
    ...
    openspec_proposal_id: Mapped[UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        # Future: ForeignKey("openspec_proposals.id", ondelete="SET NULL"),
        nullable=True,
    ...
    def __repr__(self) -> str:
        ...
        return f"<Task(id={self.id}, identifier={self.identifier}, title={self.title[:50]}, status={self.status})>"