...
class Message(BaseModel, Base):
    ...
    role: Mapped[MessageRole] = mapped_column(
        String(10), nullable=False, comment="Message role (user, assistant, system)"
    )
    ...
    model_used: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="AI model name used for assistant messages"
    )

    tokens_input: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Input tokens for AI-generated messages"
    )

    tokens_output: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Output tokens for AI-generated messages"
    )

    cost: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 6), nullable=True, comment="Cost for AI-generated messages"
    )
    ...
    # Relationships
    chat: Mapped["Chat"] = relationship("Chat", back_populates="messages", lazy="select")
