...
class EmbeddingResponse(BaseModel):
    ...
    model_name: str = Field(..., description="Model used for embedding generation")
    text_length: int = Field(..., description="Length of input text")
    processing_time_ms: float = Field(..., description="Processing time in milliseconds")
    cached: bool = Field(..., description="Whether result was retrieved from Redis cache")
    pool_cached: bool = Field(
        default=False, description="Whether result was retrieved from in-memory pool"
    )
...
class BatchEmbeddingResponse(BaseModel):
    ...
    processed_texts: int = Field(..., description="Number of texts successfully processed")
    redis_cached_count: int = Field(..., description="Number of results retrieved from Redis cache")
    pool_cached_count: int = Field(
        default=0, description="Number of results retrieved from in-memory pool"
    )
    ...
    smart_batching_used: bool = Field(
        default=False, description="Whether smart batching optimization was used"
    )
...
class SimilaritySearchResponse(BaseModel):
    ...
    collection_type: str = Field(..., description="Collection type searched")
    score_threshold: float = Field(..., description="Score threshold used for search")
...
class EmbeddingServiceInfo(BaseModel):
    ...
    redis_cache_hits: int = Field(..., description="Number of Redis cache hits")
    redis_cache_misses: int = Field(..., description="Number of Redis cache misses")
    redis_cache_hit_rate: float = Field(..., description="Redis cache hit rate (0.0 to 1.0)")
    pool_hits: int = Field(default=0, description="Number of in-memory pool hits")
    pool_misses: int = Field(default=0, description="Number of in-memory pool misses")
    pool_hit_rate: float = Field(default=0.0, description="In-memory pool hit rate (0.0 to 1.0)")
    pool_size: int = Field(default=0, description="Current in-memory pool size")
    pool_max_size: int = Field(default=0, description="Maximum in-memory pool size")
    average_time_ms: float = Field(..., description="Average processing time in milliseconds")
    smart_batching_enabled: bool = Field(
        default=False, description="Whether smart batching is enabled"
    )
    embedding_pool_enabled: bool = Field(
        default=False, description="Whether embedding pool is enabled"
    )
...
class EmbeddingHealthResponse(BaseModel):
    ...
    model_status: str = Field(..., description="Model status (loaded, not_loaded, error)")
    cache_status: str = Field(..., description="Cache status (healthy, unhealthy, disconnected)")
    generation_status: str = Field(..., description="Embedding generation status (working, failed)")
    model_name: str = Field(..., description="Model name")
    dimension: int = Field(..., description="Embedding dimension")
    timestamp: float = Field(..., description="Health check timestamp")
    error: Optional[str] = Field(default=None, description="Error message if status is unhealthy")
...
class CacheManagementResponse(BaseModel):
    ...
    collection_type: Optional[str] = Field(
        default=None, description="Collection type targeted (if applicable)"
    )
    cache_stats: Optional[Dict[str, Any]] = Field(
        default=None, description="Cache statistics (for info/stats operations)"
    )
...
class EmbeddingMetricsResponse(BaseModel):
    ...
    average_time_ms: float = Field(..., description="Average processing time per embedding")
    cache_stats: Dict[str, Union[int, float]] = Field(..., description="Cache statistics")
