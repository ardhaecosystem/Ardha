...
class CreateChatRequest(BaseModel):
    ...
    project_id: Optional[UUID] = Field(
        default=None, description="Optional project ID to associate chat with"
    )
...
class SendMessageRequest(BaseModel):
    ...
    content: str = Field(..., min_length=1, max_length=50000, description="Message content to send")
    model: str = Field(
        ..., min_length=1, max_length=100, description="AI model to use for response"
    )
