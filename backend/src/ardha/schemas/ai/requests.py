...
class ChatMessage(BaseModel):
    ...
    name: Optional[str] = Field(default=None, description="Name of the message sender (for tool messages)")
...
class CompletionRequest(BaseModel):
    ...
    max_tokens: Optional[int] = Field(default=None, ge=1, le=32768, description="Maximum tokens to generate")
    ...
    tools: Optional[List[ToolDefinition]] = Field(default=None, description="Tools available to the AI")
    tool_choice: Optional[Union[str, Dict[str, Any]]] = Field(default=None, description="Tool choice strategy")
    response_format: Optional[Dict[str, str]] = Field(default=None, description="Response format (e.g., JSON mode)")
    ...
    def validate_messages(cls, v: List[ChatMessage]) -> List[ChatMessage]:
        ...
        for i in range(1, len(v)):
            if (v[i].role == v[i-1].role and 
                v[i].role not in [MessageRole.TOOL] and 
                v[i-1].role not in [MessageRole.TOOL]):
...
class TokenCountRequest(BaseModel):
    ...
    messages: Optional[List[ChatMessage]] = Field(default=None, description="Messages to count tokens for")
    ...
    @field_validator("messages")
    @classmethod
    def validate_messages_or_text(cls, v: Optional[List[ChatMessage]], info) -> Optional[List[ChatMessage]]:
        """Validate that either text or messages is provided."""
        values = info.data
