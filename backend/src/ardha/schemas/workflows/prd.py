...
class PRDState(WorkflowState):
    ...
    prd_content: Optional[str] = Field(default=None, description="Generated PRD document content")
    final_prd: Optional[str] = Field(default=None, description="Final polished PRD document")
    ...
    # PRD quality metrics
    requirements_completeness: float = Field(
        default=0.0, description="Completeness of requirements extraction (0-1)"
    )
    feature_prioritization_quality: float = Field(
        default=0.0, description="Quality of feature prioritization (0-1)"
    )
    metrics_specificity: float = Field(
        default=0.0, description="Specificity of success metrics (0-1)"
    )
    ...
    # Progress tracking
    current_prd_step: str = Field(
        default="extract_requirements", description="Currently executing PRD step"
    )
    prd_progress_percentage: int = Field(
        default=0, description="PRD workflow progress percentage (0-100)"
    )
    ...
    def add_human_approval(
        self, approval_point: str, approved: bool, feedback: Optional[str] = None
    ) -> None:
...
class PRDStepResult(BaseModel):
    ...
    data_structures: Dict[str, Any] = Field(
        default_factory=dict, description="Structured data generated"
    )
    ...
    human_approval_granted: Optional[bool] = Field(
        default=None, description="Whether human approval was granted"
    )
...
class PRDWorkflowConfig(BaseModel):
    ...
    # Model assignments
    extract_requirements_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for requirements extraction"
    )
    define_features_model: str = Field(
        default="z-ai/glm-4.6", description="Model for feature definition"
    )
    set_metrics_model: str = Field(
        default="z-ai/glm-4.6", description="Model for metrics definition"
    )
    generate_prd_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for PRD generation"
    )
    review_format_model: str = Field(
        default="z-ai/glm-4.6", description="Model for review and formatting"
    )
    ...
    # Quality thresholds
    minimum_completeness_threshold: float = Field(
        default=0.8, description="Minimum completeness threshold"
    )
    ...
    auto_approve_confidence_threshold: float = Field(
        default=0.9, description="Auto-approve if confidence above threshold"
    )

    # PRD template settings
    include_technical_architecture: bool = Field(
        default=True, description="Include technical architecture section"
    )
    include_timeline_milestones: bool = Field(
        default=True, description="Include timeline and milestones section"
    )
    include_success_metrics: bool = Field(
        default=True, description="Include success metrics section"
    )

    # Version management
    auto_increment_version: bool = Field(
        default=True, description="Auto-increment version on changes"
    )
    version_format: str = Field(
        default="semantic", description="Version format: semantic, date, custom"
    )
...
class PRDProgressUpdate(BaseModel):
    ...
    step_completion_time: Optional[str] = Field(
        default=None, description="Step completion timestamp"
    )
    step_duration_ms: Optional[int] = Field(
        default=None, description="Step duration in milliseconds"
    )

    # Results preview
    step_result_preview: Optional[str] = Field(default=None, description="Preview of step results")
    key_outputs_preview: List[str] = Field(
        default_factory=list, description="Preview of key outputs"
    )
    ...
    completeness_score: Optional[float] = Field(
        default=None, description="Completeness score for step"
    )

    # Human interaction
    human_approval_requested: bool = Field(
        default=False, description="Whether human approval was requested"
    )
    human_approval_status: Optional[str] = Field(
        default=None, description="Status of human approval"
    )
    human_feedback: Optional[str] = Field(default=None, description="Human feedback provided")
    ...
    document_length: Optional[int] = Field(
        default=None, description="Current document length in characters"
    )

    # Overall workflow status
    total_cost_so_far: Optional[float] = Field(
        default=None, description="Total cost incurred so far"
    )
    estimated_remaining_cost: Optional[float] = Field(
        default=None, description="Estimated remaining cost"
    )
...
class PRDDocumentMetadata(BaseModel):
    ...
    functional_requirements_count: int = Field(
        default=0, description="Number of functional requirements"
    )
    non_functional_requirements_count: int = Field(
        default=0, description="Number of non-functional requirements"
    )
