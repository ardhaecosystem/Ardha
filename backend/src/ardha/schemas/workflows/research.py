...
class ResearchState(WorkflowState):
    ...
    research_confidence: float = Field(
        default=0.0, description="Overall confidence in research results"
    )
    sources_found: int = Field(default=0, description="Total number of sources/references found")
    hypotheses_generated: int = Field(
        default=0, description="Number of research hypotheses generated"
    )
    ...
    technical_detail_level: float = Field(
        default=0.0, description="Level of technical detail (0-1)"
    )
...
class ResearchWorkflowConfig(BaseModel):
    ...
    market_research_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for market research"
    )
    competitive_analysis_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for competitive analysis"
    )
    technical_feasibility_model: str = Field(
        default="z-ai/glm-4.6", description="Model for technical feasibility"
    )
    synthesis_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for research synthesis"
    )
    ...
    # Research parameters
    max_sources_per_step: int = Field(
        default=10, description="Maximum sources to consider per step"
    )
...
class ResearchProgressUpdate(BaseModel):
    ...
    key_findings_preview: List[str] = Field(
        default_factory=list, description="Preview of key findings"
    )
