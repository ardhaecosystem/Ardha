...
class TaskGenerationProgressUpdate(BaseModel):
    ...
    timestamp: str = Field(
        default_factory=lambda: datetime.utcnow().isoformat(), description="Update timestamp"
    )
...
class TaskGenerationWorkflowConfig(BaseModel):
    ...
    analyze_prd_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for PRD analysis"
    )
    breakdown_tasks_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for task breakdown"
    )
    define_dependencies_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for dependency definition"
    )
    estimate_effort_model: str = Field(
        default="z-ai/glm-4.6", description="Model for effort estimation"
    )
    generate_openspec_model: str = Field(
        default="anthropic/claude-sonnet-4.5", description="Model for OpenSpec generation"
    )

    # Workflow parameters
    max_retries_per_step: int = Field(default=3, ge=1, le=10, description="Max retries per step")
    timeout_per_step_seconds: int = Field(
        default=300, ge=60, le=1800, description="Timeout per step in seconds"
    )
    ...
    include_acceptance_criteria: bool = Field(
        default=True, description="Include acceptance criteria"
    )

    # Quality thresholds
    minimum_task_quality_score: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum task quality score"
    )
    minimum_dependency_clarity: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Minimum dependency clarity score"
    )
    minimum_effort_accuracy: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Minimum effort estimation accuracy"
    )

    # OpenSpec parameters
    openspec_template_path: Optional[str] = Field(None, description="Custom OpenSpec template path")
    include_proposal_summary: bool = Field(
        default=True, description="Include proposal summary in OpenSpec"
    )
    include_risk_assessment: bool = Field(
        default=True, description="Include risk assessment in OpenSpec"
    )
...
class TaskGenerationState(WorkflowState):
    ...
    project_context: Optional[Dict[str, Any]] = Field(
        None, description="Project context and constraints"
    )
    existing_tasks: Optional[List[Dict[str, Any]]] = Field(
        None, description="Existing tasks in project"
    )

    # Analysis results
    prd_analysis: Optional[Dict[str, Any]] = Field(None, description="Analysis of PRD content")
    feature_breakdown: Optional[List[Dict[str, Any]]] = Field(
        None, description="Feature breakdown from PRD"
    )
    technical_requirements: Optional[List[Dict[str, Any]]] = Field(
        None, description="Technical requirements extracted"
    )

    # Task breakdown results
    task_breakdown: Optional[List[Dict[str, Any]]] = Field(
        None, description="Complete task breakdown"
    )
    epics_defined: Optional[List[Dict[str, Any]]] = Field(
        None, description="Epics identified and defined"
    )
    subtasks_created: Optional[List[Dict[str, Any]]] = Field(
        None, description="Subtasks created for main tasks"
    )

    # Dependency and effort results
    task_dependencies: Optional[List[Dict[str, Any]]] = Field(
        None, description="Task dependencies defined"
    )
    dependency_graph: Optional[Dict[str, Any]] = Field(
        None, description="Complete dependency graph"
    )
    effort_estimates: Optional[Dict[str, Any]] = Field(
        None, description="Effort estimates for all tasks"
    )
    resource_allocation: Optional[Dict[str, Any]] = Field(
        None, description="Resource allocation recommendations"
    )

    # OpenSpec generation results
    openspec_proposal: Optional[Dict[str, Any]] = Field(
        None, description="Generated OpenSpec proposal"
    )
    proposal_metadata: Optional[Dict[str, Any]] = Field(
        None, description="OpenSpec proposal metadata"
    )
    change_directory_path: Optional[str] = Field(
        None, description="Path for openspec/changes/ directory"
    )
    generated_files: Optional[Dict[str, str]] = Field(
        None, description="Generated OpenSpec files mapping"
    )

    # ============= Progress Tracking =============

    current_task_step: str = Field(
        default="analyze_prd", description="Current task generation step"
    )
    task_progress_percentage: float = Field(
        default=0.0, ge=0.0, le=100.0, description="Task generation progress percentage"
    )
    completed_task_steps: List[str] = Field(
        default_factory=list, description="Completed task generation steps"
    )
    failed_task_steps: List[str] = Field(
        default_factory=list, description="Failed task generation steps"
    )

    # ============= Quality Metrics =============

    prd_analysis_quality: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Quality of PRD analysis"
    )
    task_breakdown_completeness: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Completeness of task breakdown"
    )
    dependency_accuracy: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Accuracy of dependency definitions"
    )
    effort_estimation_quality: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Quality of effort estimates"
    )
    openspec_quality_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Overall OpenSpec quality score"
    )

    # ============= Step Results =============

    step_results: List[Dict[str, Any]] = Field(
        default_factory=list, description="Results of each step"
    )

    # ============= Human Interaction Points =============

    human_approval_points: List[str] = Field(
        default_factory=list, description="Points requiring human approval"
    )
    human_edits_made: List[Dict[str, Any]] = Field(
        default_factory=list, description="Human edits recorded"
    )
    ...
    def get_task_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tasks_with_estimates": len(
                [t for t in self.task_breakdown if t.get("estimate_hours")]
            ),
            "tasks_with_dependencies": len(
                [t for t in self.task_breakdown if t.get("dependencies")]
            ),
