...
class EmbeddingSettings(BaseModel):
    ...
    enable_model_warmup: bool = Field(
        default=True,
        description="Whether to warm up the model on first load"
    )
    enable_embedding_pool: bool = Field(
        default=True,
        description="Whether to use embedding pooling for performance"
    )
    pool_size: int = Field(
        default=1000,
        ge=100,
    ...
    enable_compression: bool = Field(
        default=False,
        description="Whether to compress cached embeddings"
    )
    compression_threshold: int = Field(
        default=100,
        ge=10,
        le=1000,
        description="Minimum batch size to enable compression"
    )
    enable_smart_batching: bool = Field(
        default=True,
        description="Whether to use smart batching for optimal performance"
    )
    smart_batch_threshold: int = Field(
        default=16,
        ge=4,
        le=64,
        description="Threshold for smart batching optimization"
    )
