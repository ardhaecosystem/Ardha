...
from ..schemas.ai.models import AIModel, get_model
...
class CircuitBreaker:
    ...
    def __init__(self, threshold: int = 3, cooldown_period: int = 300):
        ...
        self.state = CircuitBreakerState(
            is_open=False, failure_count=0, cooldown_period=cooldown_period, threshold=threshold
        )
    ...
    def get_state(self) -> CircuitBreakerState:
        ...
        if self.state.is_open and self.state.next_attempt_time:
            state_data["time_until_next_attempt"] = max(
                0, int(self.state.next_attempt_time - time.time())
            )
        else:
...
class OpenRouterClient:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None):
        ...
        logger.info(
            f"OpenRouter client initialized with timeout={self.timeout}s, max_retries={self.max_retries}"
        )
    ...
    def _prepare_request_data(
        self, request: Union[CompletionRequest, StreamingRequest]
    ) -> Dict[str, Any]:
    ...
    async def complete(self, request: CompletionRequest) -> CompletionResponse:
        ...
        if completion_response.usage:
            ...
            cost = model.calculate_cost(
                completion_response.usage.prompt_tokens, completion_response.usage.completion_tokens
            )
            # Store cost in a custom attribute or log it
            logger.debug(f"Completion cost: ${cost:.6f}")

        logger.info(
            f"Completion successful, cost: ${completion_response.usage.cost if completion_response.usage else 0:.6f}"
        )
            ...
            async with self.client.stream("POST", "/chat/completions", json=data) as response:
                ...
                if "text/event-stream" in content_type:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to parse JSON response: {e}")
                        raise OpenRouterError(
                            f"Invalid JSON response: {e}", error_type="json_error"
                        )
