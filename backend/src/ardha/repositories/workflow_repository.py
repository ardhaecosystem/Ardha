    ...
    async def get_by_id(self, execution_id: UUID) -> Optional[WorkflowExecution]:
        ...
        try:
            stmt = select(WorkflowExecution).where(
                ...
                selectinload(WorkflowExecution.user),
                selectinload(WorkflowExecution.project)
            )
    ...
    async def get_execution_stats(
        ...
        try:
            ...
            return {
                ...
                'status_breakdown': {status.value if hasattr(status, 'value') else str(status): count for status, count in status_breakdown.items()},
