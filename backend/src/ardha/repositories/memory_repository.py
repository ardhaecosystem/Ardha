...
from ardha.models.memory import Memory, MemoryLink, MemoryType, SourceType, RelationshipType
...
class MemoryRepository:
    ...
    async def create(
        ...
        try:
            ...
            if memory_type not in [MemoryType.CONVERSATION, MemoryType.WORKFLOW, 
                                  MemoryType.DOCUMENT, MemoryType.ENTITY, MemoryType.FACT]:
                raise ValueError(f"Invalid memory_type: {memory_type}")
            if source_type not in [SourceType.CHAT, SourceType.WORKFLOW, 
                                 SourceType.MANUAL, SourceType.API]:
    ...
    async def update(self, memory_id: UUID, **kwargs) -> Optional[Memory]:
        ...
        try:
            ...
            allowed_fields = {
                'content', 'summary', 'importance', 'confidence', 'tags', 
                'extra_metadata', 'expires_at', 'is_archived'
            }
    ...
    async def get_important(
        ...
        try:
            ...
            logger.error(f"Error fetching important memories for user {user_id}: {e}", exc_info=True)
            raise
    ...
    async def increment_access_count(self, memory_id: UUID) -> Optional[Memory]:
        ...
        try:
            ...
            logger.error(f"Error incrementing access count for memory {memory_id}: {e}", exc_info=True)
            raise
    ...
    async def get_by_source(
        ...
        try:
            ...
            logger.error(f"Error getting memories by source {source_type}:{source_id}: {e}", exc_info=True)
            raise
    ...
    async def create_link(
        ...
        try:
            ...
            if relationship_type not in [RelationshipType.RELATED_TO, RelationshipType.DEPENDS_ON,
    ...
    async def get_related_memories(
        ...
        if relationship_type and relationship_type not in [RelationshipType.RELATED_TO, RelationshipType.DEPENDS_ON,
                                                          RelationshipType.CONTRADICTS, RelationshipType.SUPPORTS]:
            raise ValueError(f"Invalid relationship_type: {relationship_type}")
        ...
        try:
            ...
            base_conditions = [
                ...
                or_(
                    MemoryLink.memory_from_id == memory_id,
                    MemoryLink.memory_to_id == memory_id
                )
            ]
