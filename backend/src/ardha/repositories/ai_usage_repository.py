...
class AIUsageRepository:
    ...
    async def create(
        ...
        try:
            ...
            if operation not in [op.value for op in AIOperation]:
                raise ValueError(
                    f"Invalid operation: {operation}. Must be one of: {[op.value for op in AIOperation]}"
                )
    ...
    async def get_daily_usage(self, user_id: UUID, usage_date: date) -> List[AIUsage]:
        ...
        try:
            ...
            logger.error(
                f"Error fetching daily usage for user {user_id} on {usage_date}: {e}", exc_info=True
            )
            raise
    ...
    async def get_project_usage(
        ...
        try:
            ...
            logger.error(
                f"Error fetching project usage for {project_id} from {start_date} to {end_date}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def get_user_total_cost(
        ...
        try:
            stmt = select(func.coalesce(func.sum(AIUsage.cost), 0)).where(
                and_(
                    AIUsage.user_id == user_id,
                    AIUsage.usage_date >= start_date,
                    AIUsage.usage_date <= end_date,
            ...
            logger.error(
                f"Error calculating total cost for user {user_id} from {start_date} to {end_date}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def get_user_usage_stats(
        ...
        try:
            ...
            model_usage = {
                row.model_name: {"count": row.count, "cost": row.cost} for row in model_result
            }
