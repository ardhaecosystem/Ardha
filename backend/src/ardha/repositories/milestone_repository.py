...
class MilestoneRepository:
    ...
    async def get_by_status(self, project_id: UUID, status: str) -> list[Milestone]:
        ...
        try:
            ...
            logger.error(f"Error fetching milestones by status for project {project_id}: {e}", exc_info=True)
            raise
    ...
    async def create(self, milestone_data: dict) -> Milestone:
        ...
        try:
            ...
            max_order_stmt = (
                select(func.coalesce(func.max(Milestone.order), -1))
                .where(Milestone.project_id == project_id)
            )
            ...
            logger.info(f"Auto-assigning order {next_order} for project {project_id} (max_order was {max_order})")
    ...
    async def reorder(
        ...
        try:
            ...
            if milestone.project_id != project_id:
                raise ValueError(f"Milestone {milestone_id} does not belong to project {project_id}")
    ...
    async def calculate_progress(self, milestone_id: UUID) -> int:
        ...
        try:
            ...
            logger.error(f"Error calculating progress for milestone {milestone_id}: {e}", exc_info=True)
            raise
    ...
    async def get_milestones_with_task_counts(
        ...
        try:
            ...
            logger.error(f"Error fetching milestones with task counts for project {project_id}: {e}", exc_info=True)
            raise
    ...
    async def get_upcoming_milestones(
        ...
        try:
            ...
            logger.error(f"Error fetching upcoming milestones for project {project_id}: {e}", exc_info=True)
            raise
    ...
    async def get_overdue_milestones(self, project_id: UUID) -> list[Milestone]:
        ...
        try:
            ...
            logger.error(f"Error fetching overdue milestones for project {project_id}: {e}", exc_info=True)
            raise