...
class TaskGenerationRequest(BaseModel):
    ...
    config: Optional[TaskGenerationWorkflowConfig] = Field(
...
async def execute_task_generation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to start task generation workflow: {str(e)}"
        )
...
async def get_task_generation_status(
    ...
    try:
        ...
        if not state:
            raise HTTPException(
                status_code=404, detail=f"Task generation execution {execution_id} not found"
            )

        # Check permissions (user can only see their own executions)
        if state.user_id != current_user.id:
            raise HTTPException(status_code=403, detail="Access denied to this workflow execution")
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get task generation status: {str(e)}"
        )
...
async def get_task_generation_progress(
    ...
    try:
        logger.info(
            f"User {current_user.id} requesting progress stream for execution {execution_id}"
        )
            ...
            try:
                ...
                progress_data = {
                    ...
                    "status": (
                        ...
                        if hasattr(current_state.status, "value")
                        else str(current_state.status)
                    ),
                ...
                while current_state.status in ["pending", "running"] and iteration < max_iterations:
                    ...
                    if (
                        updated_state.last_activity != last_update
                        or updated_state.status != current_state.status
                    ):
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create progress stream: {str(e)}")
...
async def get_task_generation_results(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get task generation results: {str(e)}"
        )
...
async def cancel_task_generation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to cancel task generation execution: {str(e)}"
        )
...
async def get_task_generation_config(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get task generation config: {str(e)}"
        )
...
async def list_task_generation_executions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list task generation executions: {str(e)}"
        )
