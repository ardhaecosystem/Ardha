...
class WorkflowTrackingService:
    ...
    async def analyze_performance(
        ...
        try:
            ...
            analysis["success_rate"] = (successful / len(recent_executions)) * 100 if recent_executions else 0
            ...
            for execution in recent_executions:
                if execution.get("metrics", {}).get("duration_ms"):
                    durations.append(execution["metrics"]["duration_ms"] / 1000)  # Convert to seconds
    ...
    async def _update_global_metrics(
        ...
        try:
            ...
            if metrics.get("duration_ms"):
                total_duration = self.metrics_cache["average_duration"] * (self.metrics_cache["total_executions"] - 1)
                total_duration += metrics["duration_ms"] / 1000  # Convert to seconds
                self.metrics_cache["average_duration"] = total_duration / self.metrics_cache["total_executions"]
