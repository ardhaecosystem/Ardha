...
from ..schemas.workflows.prd import PRDProgressUpdate, PRDState, PRDWorkflowConfig
...
class PRDWorkflow:
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges(
            ...
            {"define_features_node": "define_features_node", "error": "error_handler", "end": END},
        )

        workflow.add_conditional_edges(
            "define_features_node",
            self._decide_next_step,
            {"set_metrics_node": "set_metrics_node", "error": "error_handler", "end": END},
        )

        workflow.add_conditional_edges(
            "set_metrics_node",
            self._decide_next_step,
            {"generate_prd_node": "generate_prd_node", "error": "error_handler", "end": END},
        )

        workflow.add_conditional_edges(
            "generate_prd_node",
            self._decide_next_step,
            {"review_format_node": "review_format_node", "error": "error_handler", "end": END},
        )

        workflow.add_conditional_edges(
            "review_format_node", self._decide_next_step, {"end": END, "error": "error_handler"}
        )

        workflow.add_conditional_edges(
            "error_handler",
            self._decide_error_recovery,
            {"retry": "extract_requirements_node", "end": END},  # Retry from start
        )
