...
class WorkflowState(BaseModel):
    ...
    results: Dict[str, Any] = Field(
        default_factory=dict, description="Accumulated results from nodes"
    )
    artifacts: Dict[str, Any] = Field(
        default_factory=dict, description="Generated artifacts (code, docs, etc.)"
    )
    ...
    def add_artifact(
        self, artifact_type: str, content: Any, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    def add_ai_call(
        self, model: str, operation: str, tokens_input: int, tokens_output: int, cost: float
    ) -> None:
...
class NodeState(BaseModel):
    ...
    output_data: Dict[str, Any] = Field(
        default_factory=dict, description="Output data from the node"
    )
...
class WorkflowContext(BaseModel):
    ...
    progress_callback: Optional[Callable] = Field(
        default=None, description="Progress update callback"
    )
