...
class TaskGenerationWorkflow:
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges(
            ...
            {"breakdown_tasks_node": "breakdown_tasks_node", "error": "error_handler", "end": END},
        )
        ...
        workflow.add_conditional_edges(
            ...
            self._decide_next_step,
            {"estimate_effort_node": "estimate_effort_node", "error": "error_handler", "end": END},
        )
        ...
        workflow.add_conditional_edges(
            "generate_openspec_node", self._decide_next_step, {"end": END, "error": "error_handler"}
        )

        workflow.add_conditional_edges(
            "error_handler",
            self._decide_error_recovery,
            {"retry": "analyze_prd_node", "end": END},  # Retry from start
        )
    ...
    async def _define_dependencies_node(
        self, state: "TaskGenerationState"
    ) -> "TaskGenerationState":
    ...
    async def _generate_openspec_node(self, state: "TaskGenerationState") -> "TaskGenerationState":
        ...
        try:
            ...
            try:
                ...
                if (
                    hasattr(state, "project_id")
                    and hasattr(state, "user_id")
                    ...
                    self.logger.warning(
                        "No project_id or user_id in state - skipping database save"
                    )
    ...
    async def _default_progress_callback(
        self, progress_update: "TaskGenerationProgressUpdate"
    ) -> None:
        """
    ...
    async def get_progress_from_redis(
        self, execution_id: UUID
    ) -> Optional["TaskGenerationProgressUpdate"]:
        """
...
def get_task_generation_workflow(
    config: Optional["TaskGenerationWorkflowConfig"] = None,
