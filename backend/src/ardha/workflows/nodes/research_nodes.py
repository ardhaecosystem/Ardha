...
from ...schemas.workflows.research import ResearchProgressUpdate, ResearchStepResult
    ...
    async def execute(
        ...
        try:
            ...
            await self._emit_progress_update(
                state, context, "running", 0, f"Starting {self.node_name} analysis..."
            )
            ...
            self.logger.error(
                f"Research node {self.node_name} failed after {execution_time_ms}ms: {e}"
            )
...
class AnalyzeIdeaNode(BaseResearchNode):
    ...
    async def _execute_research_logic(
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an expert idea analyst. Be thorough and structured.",
            },
...
class MarketResearchNode(BaseResearchNode):
    ...
    async def _execute_research_logic(
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an expert market research analyst. Use current market data and be specific with numbers.",
            },
            {"role": "user", "content": prompt},
        ]
...
class CompetitiveAnalysisNode(BaseResearchNode):
    ...
    async def _execute_research_logic(
        ...
        prompt = RESEARCH_PROMPTS["competitive_analysis"].format(
            idea=idea,
            market_research=(
                market_research[:1000] if market_research else "No market research available"
            ),
        )
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a competitive intelligence analyst. Be objective and data-driven.",
            },
            {"role": "user", "content": prompt},
        ]
...
class TechnicalFeasibilityNode(BaseResearchNode):
    ...
    async def _execute_research_logic(
        ...
        prompt = RESEARCH_PROMPTS["technical_feasibility"].format(
            idea=idea, requirements=requirements[:1500] if requirements else idea
        )

        messages = [
            {
                "role": "system",
                "content": "You are a technical feasibility analyst. Be realistic and specific about technical requirements.",
            },
            {"role": "user", "content": prompt},
        ]
...
class SynthesizeResearchNode(BaseResearchNode):
    ...
    async def _execute_research_logic(
        ...
        prompt = RESEARCH_PROMPTS["synthesize"].format(
            ...
            market_research=(
                market_research[:800] if market_research else "No market research available"
            ),
            competitive_analysis=(
                competitive_analysis[:800]
                if competitive_analysis
                else "No competitive analysis available"
            ),
            technical_feasibility=(
                technical_feasibility[:800]
                if technical_feasibility
                else "No technical feasibility available"
            ),
        )
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an expert research synthesizer. Create actionable executive summary with clear recommendations.",
            },
            {"role": "user", "content": prompt},
        ]
