...
export function ChatMessage({ message }: MessageProps) {
  ...
  const totalTokens = (message.tokens_input || 0) + (message.tokens_output || 0);
    ...
    <div className={`flex gap-4 ${isUser ? 'flex-row-reverse' : 'flex-row'} group`}>
      ...
      <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${
        ...
        {isUser ? (
          ...
          <svg className="w-6 h-6 text-[hsl(var(--neon-blue))]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        >
          ...
          <div className="flex items-center gap-3 mt-3 text-white/40 text-xs">
            ...
            {message.ai_model && (
              <>
                <span>•</span>
                <span className="text-[hsl(var(--neon-blue))]">{message.ai_model.split('/')[1] || message.ai_model}</span>
              </>
            )}
            {totalTokens > 0 && (
              <>
    ...
    <div className="flex gap-4">
      ...
      <div className="glass-panel p-4 rounded-2xl border-[hsl(var(--neon-blue))]/30">
        <div className="flex gap-2">
          <div className="w-2 h-2 rounded-full bg-[hsl(var(--neon-blue))] animate-bounce" style={{ animationDelay: '0ms' }} />
          <div className="w-2 h-2 rounded-full bg-[hsl(var(--neon-blue))] animate-bounce" style={{ animationDelay: '150ms' }} />
          <div className="w-2 h-2 rounded-full bg-[hsl(var(--neon-blue))] animate-bounce" style={{ animationDelay: '300ms' }} />
        </div>
      </div>
    </div>
  );
